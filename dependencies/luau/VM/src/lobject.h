...
typedef struct Udata
{
    ...
    u_meta_enc<struct LuaTable*> metatable;
...
typedef struct Proto
{
    ...
    LUAVM_SHUFFLE5(LUAVM_SHUFFLE_OTHER,
    ...
    LUAVM_SHUFFLE4(LUAVM_SHUFFLE_OTHER,
    ...
    LUAVM_SHUFFLE5(LUAVM_SHUFFLE_OTHER,
    p_member2_enc<uint8_t*> lineinfo,      // for each instruction, line number as a delta from baseline
    p_member2_enc<int*> abslineinfo,       // baseline line info, one entry for each 1<<linegaplog2 instructions; allocated after lineinfo
    p_member2_enc<struct LocVar*> locvars, // information about local variables
    p_member2_enc<TString**> upvalues,     // upvalue names
    p_member2_enc<TString*> source);

    p_debugname_enc<TString*> debugname;
    p_debuginsn_enc<uint8_t*> debuginsn; // a copy of code[] array with just opcodes

    p_typeinfo_enc<uint8_t*> typeinfo;
    ...
    LUAVM_SHUFFLE9(LUAVM_SHUFFLE_OTHER,
    int sizecode,       // a1
    int sizep,          // a2
    int sizelocvars,    // a3
    int sizeupvalues,   // a4
...
#define luaO_nilobject reinterpret_cast<TValue*>(rbx::luau::luao_nilobject)
