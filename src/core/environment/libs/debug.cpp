...
lua_CFunction old_getfenv = nullptr;
LuaTable *our_env = nullptr;

int getfenv_stub(lua_State *L) {
...
void environment::load_debug_lib(lua_State *L) {
    lua_pushcclosure(L, debug_getconstant, nullptr, 0);
    lua_setglobal(L, "getconstant");

    lua_pushcclosure(L, debug_getconstants, nullptr, 0);
    lua_setglobal(L, "getconstants");

    lua_pushcclosure(L, debug_setconstant, nullptr, 0);
    lua_setglobal(L, "setconstant");

    lua_pushcclosure(L, debug_getupvalues, nullptr, 0);
    lua_setglobal(L, "getupvalues");

    lua_pushcclosure(L, debug_getupvalue, nullptr, 0);
    lua_setglobal(L, "getupvalue");

    lua_pushcclosure(L, debug_setupvalue, nullptr, 0);
    lua_setglobal(L, "setupvalue");

    lua_pushcclosure(L, debug_getstack, nullptr, 0);
    lua_setglobal(L, "getstack");

    lua_pushcclosure(L, debug_setstack, nullptr, 0);
    lua_setglobal(L, "setstack");

    lua_pushcclosure(L, debug_getprotos, nullptr, 0);
    lua_setglobal(L, "getprotos");

    lua_pushcclosure(L, debug_getproto, nullptr, 0);
    lua_setglobal(L, "getproto");

    lua_pushcclosure(L, debug_getinfo, nullptr, 0);
    lua_setglobal(L, "getinfo");

    lua_pushcclosure(L, debug_getregistry, nullptr, 0);
    ...
    lua_getglobal(L->global->mainthread, "debug");
    lua_pushnil(L->global->mainthread);

    while (lua_next(L->global->mainthread, -2) != 0) {
        const char *key = lua_tolstring(L->global->mainthread, -2, nullptr);
        Closure *closure = clvalue(luaA_toobject(L->global->mainthread, -1));
    ...
    lua_getglobal(L, OBF("newcclosure"));
    Closure *newcc = clvalue(luaA_toobject(L, -1));
    our_env = newcc->env;
    lua_pop(L, lua_gettop(L));

    lua_getglobal(L, OBF("getfenv"));
    Closure *getfenv_closure = clvalue(luaA_toobject(L, -1));
    old_getfenv = getfenv_closure->c.f;
    getfenv_closure->c.f = getfenv_stub;
    lua_pop(L, lua_gettop(L));
}